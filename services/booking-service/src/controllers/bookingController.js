...
const { acquireSeatLocks, releaseSeatLocks } = require("../utils/seatLock");
...
const createBooking = async (req, res) => {
    ...
    try {
        locks = await acquireSeatLocks(showtimeId, seatIds);
    } catch (err) {
        logger.warn("Booking failed: could not acquire seat locks", { 
            require,
            userId,
            showtimeId,
            seatIds,
    ...
    try {
        ...
        try {
            ...
            if (err.name === "SequelizeUniqueConstraintError"){
                logger.warn("Booking failed: race condition caught", {requestId, userId, showtimeId, seatIds});
            } else {
                logger.error("Booking transaction failed", { requestId, userId, showtimeId, error: err.message});
            }
            throw new AppError(errorMessages.SEAT.ALREADY_BOOKED, 409)
        }
        ...
        if (!payment.success) {
            logger.warn("Booking failed: payment declined", {
                requestId,
                userId,
                bookingId: booking.id,
                amount: totalPrice,
